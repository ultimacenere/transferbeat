...
ALTRE FONTI, solo per il contorno e mai per i numeri: `git show origin/main:data/it/board.json` (notizie già classificate per squadra: `colonne` = {done, conf, obj, rumor}; done = fatti, conf = atti ufficiali, obj = anteprime, rumor = voci, da chiamare voci) per sapere se sul giocatore c'è una notizia fresca da citare con la sua fonte. `git show origin/main:data/teams.json` per `lab`, `col` e `league` del club. Risultati e classifiche solo dal dossier.
...
5. PUBBLICA eseguendo `bash scripts/pubblica.sh` (pubblica via token, non stampa nulla di sensibile). Se il push è rifiutato perché origin è avanzato, torna al passo 3: rifai fetch, riverifica l'elenco e ricostruisci il commit sopra il nuovo `origin/main`.
...
Se il dossier è povero (giocatore appena arrivato, pochi minuti, mezze tabelle con "non disponibile"), pezzo più corto e onesto: si dice che i dati sono ancora pochi e perché. Meglio corto che inventato. Output: giocatore scelto, titolo, lunghezza del corpo e conferma pubblicazione.
