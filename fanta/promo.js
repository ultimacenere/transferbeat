...
(function(){
  ...
  var landing = base + 'fantatb.html?lang=' + lang, app = base + 'fanta/#crea';
  var css = '.tbp-top{background:linear-gradient(100deg,#ff2e88,#ff7a1a 55%,#ffd400);color:#1b1140;font-family:"Segoe UI",system-ui,sans-serif;position:relative}'+
    ...
    '.tbp-top .x{position:absolute;right:14px;top:10px;background:rgba(27,17,64,.35);color:#fff;border:0;border-radius:50%;width:26px;height:26px;cursor:pointer;font-size:14px;line-height:26px;text-align:center}'+
    ...
    '.tbp-side .cta{display:block;background:#1b1140;color:#fff;font-weight:800;font-size:12.5px;padding:9px 6px;border-radius:8px;margin-top:8px}.tbp-side .cta:hover{background:#000}'+
  ...
  var side = function(cls){ return '<aside class="tbp-side '+cls+'"><div class="logo">Fanta<b>TB</b></div><div class="k">'+esc(T.kicker)+'</div><ul><li>'+esc(T.side1)+'</li><li>'+esc(T.side2)+'</li><li>'+esc(T.side3)+'</li></ul><a class="cta" href="'+app+'">'+esc(T.cta)+'</a><span class="free">100% GRATIS</span></aside>'; };
  var top = '<div class="tbp-top" id="tbpTop"><div class="w"><div class="t"><div class="k">'+esc(T.kicker)+'</div><div class="h">'+esc(T.h)+'</div><div class="p">'+esc(T.p)+'</div></div><a class="cta" href="'+app+'">'+esc(T.cta)+'</a><a class="more" href="'+landing+'">'+esc(T.more)+' →</a></div><button class="x" title="'+esc(T.close)+'" aria-label="'+esc(T.close)+'">×</button></div>';
