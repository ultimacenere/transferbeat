...
-- ==================== FIX 010 (risultati live, identico a fix-010-live.sql) ====================
...
-- ==================== FIX 011 (modificatore difesa configurabile, identico a fix-011-modificatori.sql; sostituisce compute_matchday del fix 010) ====================
-- FantaTB fix 011: modificatore difesa configurabile come su Fantacalcio.it (richiesta dell'utente, 2026-09-06).
-- Nuove chiavi in leagues.settings: mod_difesa_tab (tabella soglie [{min, v}], default 6:+0,5 6,25:+1 6,5:+2 6,75:+3 7:+4,5
-- 7,25:+6 7,5:+7,5), mod_difesa_portiere (true = media dei 3 migliori difensori + portiere; false = 4 migliori difensori),
-- mod_difesa_applica ('propria' = bonus alla propria squadra, 'avversaria' = malus alla squadra avversaria).
-- Serve sempre almeno 4 difensori con voto (più il portiere se incluso). Include il fix 010 (risultati live).
create or replace function public.mod_lookup(avg_v numeric, tab jsonb) returns numeric
language sql immutable as $$
  select coalesce((select (e->>'v')::numeric from jsonb_array_elements(coalesce(tab, '[]'::jsonb)) e
                   where (e->>'min')::numeric <= avg_v order by (e->>'min')::numeric desc limit 1), 0);
$$;
...
begin
  ...
  for m in select user_id from league_members where league_id = p_league loop
    ...
    if has_lu then
      foreach orig in array lu.starters loop
        ...
        select role into r from players where id = pid;
        select voto, fantavoto, bonus, minutes into v, fv, b, mins from fv_of(p_league, l.season, p_matchday, pid);
        -- live (fix 010): chi non ha ancora una riga di voto (partita non finita) resta "in attesa" e non viene sostituito;
        -- a giornata rated (tutte le partite finite) chi manca del tutto (non convocato) viene sostituito come prima
        select exists(select 1 from player_ratings where season = l.season and matchday = p_matchday and player_id = pid) into played;
        if fv is null and (md_status = 'rated' or played) and subs < max_subs and lu.bench is not null then
          foreach bp in array lu.bench loop
      ...
      if clean_b <> 0 then
        ...
        extras := extras || jsonb_build_object('k', 'porta_inviolata', 'label', 'Porta inviolata', 'v', modv); end if;
      if mod_def then
        -- fix 011: tabella configurabile, con o senza portiere, bonus alla propria squadra o malus all'avversaria
        modv := 0;
  ...
  for fx in select * from league_fixtures where league_id = p_league and matchday = p_matchday loop
    ...
    if mod_def and def_app = 'avversaria' then
      -- il modificatore della difesa di una squadra toglie punti all'avversaria
...
-- ==================== FIX 012 (6 politico nelle giornate live, identico a fix-012-sei-politico.sql; sostituisce compute_matchday del fix 011) ====================
